...
#[macro_export]
macro_rules! arena2_empty_trace {
    () => {
        #[inline]
        unsafe fn trace(&self, _color: $crate::collectors::mark_sweep_arena2::TraceColor) {}
        #[inline]
        fn run_finalizer(&self) {
            $crate::collectors::mark_sweep_arena2::Finalize::finalize(self);
        }
    };
}
...
macro_rules! arena2_custom_trace {
    ($this:ident, $marker:ident, $body:expr) => {
        ...
        #[inline]
        fn run_finalizer(&self) {
            fn $marker<T: $crate::collectors::mark_sweep_arena2::Trace + ?Sized>(it: &T) {
                $crate::collectors::mark_sweep_arena2::Trace::run_finalizer(it);
            }
            $crate::collectors::mark_sweep_arena2::Finalize::finalize(self);
            let $this = self;
            $body
...
macro_rules! simple_empty_finalize_trace {
    ($($T:ty),*) => {
        $(
            ...
            unsafe impl Trace for $T { arena2_empty_trace!(); }
        )*
    }
}
...
macro_rules! fn_finalize_trace_one {
    ($ty:ty $(,$args:ident)*) => {
        ...
        unsafe impl<Ret $(,$args)*> Trace for $ty { arena2_empty_trace!(); }
    }
}
...
macro_rules! tuple_finalize_trace {
    () => {}; // This case is handled above, by simple_finalize_arena2_empty_trace!().
    ($($args:ident),*) => {
        impl<$($args),*> Finalize for ($($args,)*) {}
        // SAFETY:
        // All elements inside the tuple are correctly marked.
        unsafe impl<$($args: $crate::collectors::mark_sweep_arena2::Trace),*> Trace for ($($args,)*) {
            arena2_custom_trace!(this, mark, {
