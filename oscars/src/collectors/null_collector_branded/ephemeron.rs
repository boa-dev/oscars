...
impl<'id, K: Trace + ?Sized, V: Trace> Ephemeron<'id, K, V> {
    ...
    /// Returns the key if still live, or `None` if it has been collected.
    ///
    /// Note: Under the null collector this always returns `Some` since nothing is ever collected
    ...
    ///
    /// Note: under the null collector this always returns true, nothing is ever collected.
    /// TODO: Must return `false` once the collector clears `key_ptr`
