...
#[test]
fn collector_drop_does_not_drop_values_revived_by_finalizer() {
    use crate::collectors::mark_sweep::internals::{GcBox, NonTraceable};
    use core::ptr::NonNull;
    use core::sync::atomic::{AtomicU32, Ordering};

    static TARGET_DROPS: AtomicU32 = AtomicU32::new(0);
    static REVIVER_DROPS: AtomicU32 = AtomicU32::new(0);

    TARGET_DROPS.store(0, Ordering::SeqCst);
    REVIVER_DROPS.store(0, Ordering::SeqCst);

    struct DropSpy;

    impl Drop for DropSpy {
        fn drop(&mut self) {
            TARGET_DROPS.fetch_add(1, Ordering::SeqCst);
        }
    }
    ...
    struct Reviver {
        target_ptr: NonNull<GcBox<NonTraceable>>,
    }

    impl Drop for Reviver {
        fn drop(&mut self) {
            REVIVER_DROPS.fetch_add(1, Ordering::SeqCst);
        }
    }

    impl Finalize for Reviver {
        fn finalize(&self) {
            // Resurrect `target` by incrementing its root count during finalization.
            unsafe { self.target_ptr.as_ref().inc_roots() };
        }
    }

    // SAFETY: `Reviver` contains no traceable fields.
    unsafe impl Trace for Reviver {
        crate::empty_trace!();
    }
    {
        let collector = &mut MarkSweepGarbageCollector::default()
        ...
        let target = Gc::new_in(DropSpy, collector);

        let reviver = Gc::new_in(
            Reviver {
                target_ptr: target.as_sized_inner_ptr(),
            },
            collector,
        );
    ...
    assert_eq!(
        TARGET_DROPS.load(Ordering::SeqCst),
        0,
        "collector drop must not drop values revived during finalization"
    );
    assert_eq!(
        REVIVER_DROPS.load(Ordering::SeqCst),
        1,
        "collector drop should still drop unreachable values after second mark"
    );
}
